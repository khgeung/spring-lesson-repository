...
@Service // 비즈니스 계층, Service Layer, 업무 정의(작업 단위), 트랜잭션 처리
@Transactional // AOP 기반 트랜잭션 처리 : 문제 발생 시 자동 롤백, 정상 수행 커밋
//위의 @Transactional을 클래스 단위에서 설정하면 하위의 모든 메소드들이 트랜잭션 관리가 된다
public class CustomerService {
	...
	private void validateCustomerInput(Customer customer) {
		if (customer == null) {
			// 아래 IllegalArgumentException 은 RuntimeException의 하위 
			// Spring AOP 기반 트랜잭션 적용 시에는 RuntimeException 계열의 
			// Exception이 발생할 때 자동 롤백되는 구조
			throw new IllegalArgumentException("고객 정보가 없습니다");
		}
		//null 인 경우 or null은 아닌데 공란으로 들어오는 경우(스페이스바)
	 ...
	 * 위 설정이 명시된 메소드는 트랜잭션 상에서 데이터 변경을 하지 않을 것임을 알림
