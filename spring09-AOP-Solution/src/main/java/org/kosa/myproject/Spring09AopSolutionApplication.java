...
/**
 * 요구사항 -> AOP를 적용하여 공통 기능을 처리 또는 횡단 관심사를 처리
 * spring08-AOP-basic 에서 AOP를 사용하지 않은 예와 비교해본다
 * 
 * 요구사항 -> AOP로 처리할 공통 계획 또는 횡단 관심사항(Cross Cutting Concern)
 * 현재 서비스 중인 시스템의 검색 계열(find 계열) 기능 메소드들을 대상으로
 * 특정 업무를 메소드 코어 작업 실행 직전에 수행하도록 작업을 추가해야 한다
 * 대상 서비스 클래스들과 메소드들이 다수가 존재하는 것으로 전제하고
 ...
 * 
	...
	2. 이후 공통 작업 (Cross Cutting Concern 공통 횡단 관심사 or 공통 관심사)의 업무가 변경되어도 
	   ...
	   높은 응집도 high cohesion : Core 은 Core 에만 집중, Cross cutting 은 별도로 전담
...
public class Spring09AopSolutionApplication implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		testMs.findMemberByAddress();//AOP적용 안됨
		...
		System.out.println(memberService.getClass().getName());//CGLIB Proxy 대리인 객체
