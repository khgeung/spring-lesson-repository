...
public class CommonLoggingAspect {
	//@Before Advice : AOP 적용 시점. Core 즉 타겟 메소드 실행 전에 AOP 기능이 수행
	//@After Advice : AOP 적용 시점. Core 실행 후 적용 
	/*
	 	execution() : pointcut 적용 AOP 적용 대상 지정
	 	public : 메소드 접근 제어자
	    ...
	    *Service: Service 로 마치는 인터페이스 또는 클래스들을 대상으로 
	    find* : find 시작하는 메소드들을 대상으로 
	   	(..): 매개변수 0..*
	 */
	@After("execution(public * org.kosa.myproject.model.*Service.find*(..))")
	public void loggin(JoinPoint point) {//JoinPoint : 실제 수행되는 core 대상 정보를 저장
		String className = point.getTarget().getClass().getName();// 실제 core 대상 클래스명 
		String methodName = point.getSignature().getName(); // 실제 core 대상 메소드명
