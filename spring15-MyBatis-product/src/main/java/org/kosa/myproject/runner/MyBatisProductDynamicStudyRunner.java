...
public class MyBatisProductDynamicStudyRunner implements CommandLineRunner {
	...
	void testDynamicUpdate() {
		...
		System.out.println("***Dynaimc SQL을 이용한 수정 테스트***");
		// 먼저 업데이트할 상품을 조회한다
	...
	void testDynamicFind() {
		System.out.println("***Dynaimc SQL을 이용한 검색 테스트***");
...
//		printSearchResult(list);
		...
		/*
		 * Mapper.xml 에서 <= 이부분이 xml tag가 아니라 <![CDATA[ 영역 ]]> Character Data 임을 알려준다
		 * <if test="minPrice != null and minPrice !=''"> <![CDATA[ AND price <=
		 * #{minPrice} ]]> </if>
		 */
	...
	void printSearchResult(List<Product> products) {
		...
		if (products.isEmpty()) {
			...
			products.forEach(product -> System.out
					.println(product.getProductName() + " " + product.getMaker() + " " + product.getPrice()));
		}
	}
