...
public class AroundAdviceStudyAspect {
	 ...
	 * advice를 모두 around advice 로 처리할 수 있다 before : 메소드 실행 전 after : 메소드 실행 후 (예외
	 * 발생과 관계없이 항상 수행) after-returning : 메소드 정상 실행 후 값이 리턴될 때 after-throwing :
	...
	public Object testAround(ProceedingJoinPoint point) throws Throwable {
		...
		// logger.debug("before advice 메소드 실행 전 횡단 관심사 로직 적용");
		String className = point.getTarget().getClass().getName();
		String methodName = point.getSignature().getName();
		logger.debug("**AOP Before**class:{} method:{}", className, methodName);
		Object retValue = null; // 실제 Core 메소드를 실행: 매우 중요한 코드 라인
		try {
		...
		// 즉 @Aspect, @Component 를 전부 주석처리하고 위 point.proceed();도 주석처리하면 Core만 실행됨
		// @Aspect, @Component 를 적용하고 point.proceed(); 를 적용하면 core, proxy 전부 실행된다.
		// 위 상황에서 point.proceed(); 를 실행하지 않으면 core 실행 x
