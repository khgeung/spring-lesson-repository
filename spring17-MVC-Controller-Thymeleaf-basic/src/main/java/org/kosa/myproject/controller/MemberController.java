...
//해당 컨트롤러의 모든 메소드 URL 앞에 "/member"가 자동으로 붙음
...
public class MemberController {
	...
	public String memberTest() {
		...
		// src/main/resource/templates/viewName(컨트롤러 반환 view 이름).html 을 찾아서
		// Thymeleaf Parser 가 동작, 클라이언트가 응답받은 일반 html을 생성해 응답한다
		// 아래의 view name 은 src/main/resource/templates/member/member-test.html을
		// 찾아서 파싱한다
		return "/member/member-test";
	}

	@GetMapping("/find-by-id") // url pattern
	// client 에서 보낸 텍스트 입력폼의 name과 매개변수명이
	// 일치하면 request.getParameter("memberId"); 처리를 해준다
	// @RequestParam : name 이 매개변수명과 일치하면 생략 가능
	public String findMemberById(@RequestParam String memberId, Model model) {
		...
		// src/main/resource/templates/member/member-test.html 로 Thymeleaf viewResolver가
		// 찾아서 파싱하고 렌더링한다
	...
	public String paramTest(@RequestParam String nick, @RequestParam int age, Model model) { // int 로 자동 형변환 : Handler
																								// Adapter 가 변환해 전달
		String type = null;
		if (age > 18) {
			type = "성인";
	...
	public String register(Member member) {
		//form 에서 전송한 회원 정보가 객체로 생성되어 매개변수로 전달된다 : Handler Adapter
